...
from udapter.dataset_readers.lemma_edit import apply_lemma_rule
from udapter.udapter_models.linear import LinearWithPGN
from udapter.udapter_models.time_distributed import TimeDistributedPGN
...
@Model.register("udapter_tag_decoder")
class TagDecoder(Model):
    ...
    def __init__(self,
        ...
        if self.adaptive:
            ...
            self.task_output = TimeDistributedPGN(LinearWithPGN(png_params_dim, self.output_dim, self.num_classes))

        self.feature_outputs = torch.nn.ModuleDict()
        self.features_metrics = {}
        for feature in self.features:
            self.feature_outputs[feature] = TimeDistributedPGN(LinearWithPGN(png_params_dim, self.output_dim,
    ...
    def _adaptive_loss(self, hidden, mask, gold_tags, output_dim, pgn_vector):
        logits = hidden
    ...
    def _loss(self, hidden, mask, gold_tags, output_dim, pgn_vector):
        logits = self.task_output(hidden, pgn_vector)
