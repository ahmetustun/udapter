...
@MatrixAttention.register("udapter-bilinear")
class BilinearMatrixAttentionWithPGN(MatrixAttention):
    """
    ...
    def __init__(self,
        ...
        if label_dim == 1:
            self._weight_matrix = Parameter(torch.Tensor(in_params, matrix_1_dim, matrix_2_dim))
        else:
            self._weight_matrix = Parameter(torch.Tensor(in_params, label_dim, matrix_1_dim, matrix_2_dim))
    ...
    @overrides
    def forward(self, matrix_1: torch.Tensor, matrix_2: torch.Tensor, pgn_vector: torch.Tensor) -> torch.Tensor:
        ...
        ex_shape = self._weight_matrix.shape
        weight = torch.matmul(pgn_vector, self._weight_matrix.view(self.in_params, -1)).view(ex_shape[1:])
