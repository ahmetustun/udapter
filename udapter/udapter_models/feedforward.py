...
class FeedForwardWithPGN(torch.nn.Module, FromParams):
    """
    ...
    def __init__(self,
        ...
        super(FeedForwardWithPGN, self).__init__()
        ...
        linear_layers = []
        param_dims = [param_dim]
        for layer_param_dim, layer_input_dim, layer_output_dim in zip(param_dims, input_dims, hidden_dims):
            linear_layers.append(LinearWithPGN(layer_param_dim, layer_input_dim, layer_output_dim))
    ...
    def forward(self, inputs: torch.Tensor, png_vector: torch.Tensor) -> torch.Tensor:
        # pylint: disable=arguments-differ
        output = inputs
        for layer, activation, dropout in zip(self._linear_layers, self._activations, self._dropout):
            output = dropout(activation(layer(output, png_vector)))
        return output
