...
@Model.register("udapter_model")
class UdapterModel(Model):
    """
    ...
    def __init__(self,
        ...
        if not self.seperate_lang_emb:
            ...
            config = self.text_field_embedder.token_embedder_bert.language_embedder.config
            language_emb_from_features = self.text_field_embedder.token_embedder_bert.language_emb_from_features
            if language_emb_from_features:
                in_lang_list = self.text_field_embedder.token_embedder_bert.language_embedder.in_language_list
                oov_lang_list = self.text_field_embedder.token_embedder_bert.language_embedder.oov_language_list
                letter_codes = self.text_field_embedder.token_embedder_bert.language_embedder.letter_codes
                self.language_embedder = LanguageMLP(config, in_lang_list, oov_lang_list, letter_codes)
            else:
                self.language_embedder = LanguageEmbeddings(config)
    ...
    def forward(self,
        ...
        for task in self.tasks:
            ...
            if task == "deps":
                ...
                pred_output = self.decoders[task](decoder_input, mask, language_emb, tag_logits,
                                                  gold_tags, metadata)
    ...
    def _apply_token_dropout(self, tokens):
        ...
        # BERT token dropout
        if "bert" in tokens:
            oov_token = self.tokenizer.vocab["[MASK]"]
            ignore_tokens = [self.tokenizer.vocab["[PAD]"], self.tokenizer.vocab["[CLS]"], self.tokenizer.vocab["[SEP]"]]
