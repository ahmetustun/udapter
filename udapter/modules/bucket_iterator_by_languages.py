...
def group_by_lang(instances: List[Instance]) -> List[Instance]:
    d = defaultdict(list)

    for item in instances:
        d[item['langs'][0]].append(item)
...
@DataIterator.register("lang-bucket")
class BucketIterator(DataIterator):
    ...
    def __init__(self,
                 ...
                 shuffle_lang_batches: bool = False) -> None:
        self.shuffle_lang_batches = shuffle_lang_batches
        if not sorting_keys:
    ...
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool) -> Iterable[Batch]:
        all_batches = []
        instances_by_lang = group_by_lang(instances)
        for instances in instances_by_lang:
    ...
    def _create_batches(self, instances: Iterable[Instance], shuffle: bool) -> Iterable[Batch]:
        ...
        instances_by_lang = group_by_lang(instances)
        if shuffle:
            random.shuffle(instances_by_lang)
        for instances in instances_by_lang:
            for instance_list in self._memory_sized_lists(instances):
                ...
                if move_to_front:
                    batches.insert(0, penultimate_batch)
                    batches.insert(0, last_batch)
                if not self.shuffle_lang_batches:
                    yield from batches
                else:
                    all_batches.extend(batches)
        if self.shuffle_lang_batches:
            random.shuffle(all_batches)
            yield from all_batches
