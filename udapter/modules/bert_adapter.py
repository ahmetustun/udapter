...
from adapter_bert.tokenization import BertTokenizer
from adapter_bert.modeling import BertModel, BertConfig
...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def __init__(self,
                 vocab: Dict[str, int],
                 in_lang_dict: Dict[str, int],
                 oov_lang_dict: Dict[str, int],
                 wordpiece_tokenizer: Callable[[str], List[str]],
                 ...
                 oov_proxy_dict: Dict[str, str] = None) -> None:

        super().__init__(token_min_padding_length)
        self.vocab = vocab
        self.in_lang_dict = in_lang_dict
        self.oov_lang_dict = oov_lang_dict
        self.oov_proxy_dict = oov_proxy_dict
    ...
    def tokens_to_indices(self,
        ...
        lang_id = [token.ent_type_ for token in tokens][0]
        lang_ids = torch.LongTensor([self.in_lang_dict[lang_id]]) if lang_id in self.in_lang_dict else torch.LongTensor([1000+self.oov_lang_dict[lang_id]])
        if lang_id in self.in_lang_dict:
            lang_ids = torch.LongTensor([self.in_lang_dict[lang_id]])
        elif self.oov_proxy_dict is not None:
            lang_ids = torch.LongTensor([self.in_lang_dict[self.oov_proxy_dict[lang_id]]])
        else:
            torch.LongTensor([1000 + self.oov_lang_dict[lang_id]])
        ...
        return {
            ...
            "mask": mask,
            f"{index_name}-lang-ids": lang_ids
        }
...
@TokenIndexer.register("udapter-bert")
class PretrainedBertIndexer(WordpieceIndexer):
    ...
    def __init__(self,
        ...
        in_lang_dict = dict()
        with open(in_lang_list) as f:
            for l in f:
                in_lang_dict[l.rstrip()] = len(in_lang_dict)

        oov_lang_dict = dict()
        with open(oov_lang_list) as f:
            for l in f:
                oov_lang_dict[l.rstrip()] = len(oov_lang_dict)

        oov_proxy_dict = None
        if oov_proxy_list is not None:
            with open(oov_proxy_list) as file:
                oov_proxy_dict = json.load(file)
        ...
        super().__init__(vocab=bert_tokenizer.vocab,
                         ...
                         in_lang_dict=in_lang_dict,
                         oov_lang_dict=oov_lang_dict,
                         oov_proxy_dict=oov_proxy_dict)
...
class BertEmbedder(TokenEmbedder):
    ...
    def __init__(self,
                 bert_model: BertModel,
                 language_embedder: LanguageMLP,
                 lang_drop_rate: float = 0.2,
                 max_pieces: int = 512,
                 start_tokens: int = 1,
                 end_tokens: int = 1,
        ...
        self.lang_drop_rate = lang_drop_rate
...
@TokenEmbedder.register("udapter-bert")
class UdifyPretrainedBertEmbedder(BertEmbedder):
    def __init__(self, pretrained_model: str,
                 bert_config_file: str,
                 in_languages: str,
                 oov_languages: str,
                 bert_requires_grad: bool = False,
                 adapters_requires_grad: bool = False,
                 lang_drop_rate: float = 0.2,
                 dropout: float = 0.1,
                 ...
                 num_adapters: int = 1,
                 use_language_emb: bool = True,
                 language_emb_size: int = 16,
                 language_emb_dropout: float = 0.1,
                 num_languages: int = 1,
                 language_one_hot: bool = False,
        ...
        bert_config.use_language_emb = use_language_emb
        bert_config.language_emb_size = language_emb_size
        bert_config.language_emb_dropout = language_emb_dropout
        bert_config.num_languages = num_languages
        bert_config.num_language_features = num_language_features
        bert_config.low_rank_dim = language_emb_size
        bert_config.language_drop_rate = language_drop_rate
        ...
        if language_emb_from_features:
            language_embedder = LanguageMLP(bert_config, in_lang_list, oov_lang_list, l2v_letter_codes)
        else:
            language_embedder = LanguageEmbeddings(bert_config)

        for name, param in model.named_parameters():
            if 'adapter' in name or 'language_emb' in name:
                param.requires_grad = adapters_requires_grad
            else:
...
@TokenEmbedder.register("udapter-bert-predictor")
