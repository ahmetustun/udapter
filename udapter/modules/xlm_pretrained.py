...
from pytorch_transformers import XLMModel, XLMTokenizer, XLMConfig
...
_NEVER_LOWERCASE = ['[UNK]', '[SEP]', '[PAD]', '[CLS]', '[MASK]', '<s>', '</s>', '<unk>', '<pad>', '<special1>']
...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def __init__(self,
                 ...
                 lang_dict: Dict[str, int] = None) -> None:

        super().__init__(token_min_padding_length)

        self.vocab = json.load(open(vocab), object_pairs_hook=OrderedDict)
    ...
    def tokens_to_indices(self,
        ...
        langs = [token.ent_type_ for token in tokens]
        ...
        token_wordpiece_lang_ids = [self.lang_dict[lang] for tokens,lang in zip(token_wordpiece_ids,langs) for _ in tokens]
        ...
        if len(flat_wordpiece_ids) <= window_length:
            ...
            # for start and end pieces, lang_id of the first token is used
            lang_id_windows = [[token_wordpiece_lang_ids[0]] + token_wordpiece_lang_ids + [token_wordpiece_lang_ids[0]]]
...
@TokenIndexer.register("udify-xlm")
class PretrainedXLMIndexer(WordpieceIndexer):
...
class XLMEmbedder(TokenEmbedder):
    """
    ...
    def forward(self,
        ...
        lang_ids : `torch.LongTensor``
            The (batch_size, ..., max_sequence_length) tensor of token language ids for XLM.
        ...
        if needs_split:
            ...
            split_lang_ids = list(lang_ids.split(self.max_pieces, dim=-1))
            ...
            split_lang_ids[-1] = F.pad(split_lang_ids[-1], pad=[0, padding_amount], value=0)

            # Now combine the sequences along the batch dimension
            input_ids = torch.cat(split_input_ids, dim=0)
            lang_ids = torch.cat(split_lang_ids, dim=0)
...
@TokenEmbedder.register("udify-xlm")
class UdifyPretrainedXLMEmbedder(XLMEmbedder):
...
@TokenEmbedder.register("udify-xlm-predictor")
class UdifyPredictionBertEmbedder(XLMEmbedder):
