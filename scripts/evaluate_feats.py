...
REF_COLUMNS = "ID FORM LEMMA UPOS XPOS FEATS HEAD DEPREL DEPS MISC LANG".split()
OUT_COLUMNS = "ID FORM LEMMA UPOS XPOS FEATS HEAD DEPREL DEPS MISC".split()
ref_ConlluRow = namedtuple("ConlluRow", REF_COLUMNS)
out_ConlluRow = namedtuple("ConlluRow", OUT_COLUMNS)
SEPARATOR = "|"
...
def manipulate_data(pairs):
    ...
    for r, o in pairs:
        ...
        # "_" prediction is also taken into account here !!
    ...
    return 100 * morph_acc / count, 100 * f1
...
def input_pairs(reference, output):
    for r, o in zip(reference, output):
        if r.count("\t") > 0:
            r_conllu = ref_ConlluRow._make(r.split("\t"))
            o_conllu = out_ConlluRow._make(o.split("\t"))
            assert r_conllu.FORM == o_conllu.FORM, (r, o)
