...
class AdapterWithParameterGen(nn.Module):
    def __init__(self, config):
        ...
        self.down_W = nn.Parameter(torch.zeros(config.low_rank_dim, config.hidden_size, config.adapter_size))
        self.down_b = nn.Parameter(torch.zeros(config.low_rank_dim, config.adapter_size))

        self.activation = ACT2FN[config.adapter_act] if isinstance(config.adapter_act, str) else config.adapter_act

        self.up_W = nn.Parameter(torch.zeros(config.low_rank_dim, config.adapter_size, config.hidden_size))
        self.up_b = nn.Parameter(torch.zeros(config.low_rank_dim, config.hidden_size))
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, adapter_idx=None, lang_embedding=None):
